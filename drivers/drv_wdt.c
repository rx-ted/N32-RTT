...
static rt_err_t n32g4fr_wdog_close(rt_watchdog_t *wdt)
{
    rt_uint32_t level;
...
static rt_err_t n32g4fr_wdog_open(rt_watchdog_t *wdt, rt_uint16_t oflag)
{
    rt_uint32_t level;
...
static rt_err_t n32g4fr_wdog_init(rt_watchdog_t *wdt)
{
    return RT_EOK;
}

static rt_err_t n32g4fr_wdog_refresh(rt_watchdog_t *wdt)
{
    rt_uint32_t level;
 ...
 *
 */
static rt_err_t n32g4fr_wdog_control(rt_watchdog_t *wdt, int cmd, void *args)
{
    RT_ASSERT(wdt != NULL);
    ...
    switch (cmd)
    {
        ...
        case RT_DEVICE_CTRL_WDT_KEEPALIVE:
        {
            ...
            n32g4fr_wdog_refresh(wdt);
        }
        break;
        case RT_DEVICE_CTRL_WDT_START:
        {
            n32g4fr_wdog_open(wdt, *(rt_uint32_t *)args);
            last_tick = rt_tick_get();
        }
        break;
        case RT_DEVICE_CTRL_WDT_STOP:
        {
            n32g4fr_wdog_close(wdt);
...
static struct rt_watchdog_ops n32g4fr_wdog_ops =
{
    .init = n32g4fr_wdog_init,
    .control = n32g4fr_wdog_control,
};

static struct rt_watchdog_device n32g4fr_wdt_device;
...
int rt_hw_wdt_init(void)
{
    ...
    /* Disable the LSI OSC */
    RCC_EnableLsi(DISABLE);
    n32g4fr_wdt_device.ops = &n32g4fr_wdog_ops;
    /* register watchdog device */
    result = rt_hw_watchdog_register(&n32g4fr_wdt_device, "wdt", RT_DEVICE_FLAG_RDWR, (void *)IWDG);
